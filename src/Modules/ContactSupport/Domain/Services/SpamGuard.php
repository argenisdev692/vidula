...
namespace Modules\ContactSupport\Domain\Services;

use Modules\ContactSupport\Domain\ValueObjects\SpamAssessment;

/**
 * Framework-free content spam scorer for the public contact form. This is the
 * second line of defence AFTER the spatie/laravel-honeypot bot layer: honeypot
 * silently drops bots, SpamGuard scores the human-readable content of requests
 * that pass. It never hard-blocks — it returns a {@see SpamAssessment} so the
 * caller can store the message flagged for review (near-zero false positives by
 * design; "help"/"support" are intentionally NOT in the keyword list).
 *
