...
final readonly class Password
{
    ...
    public function strength(): string
    {
        ...
        if (strlen($this->value) >= 8) $score++;
        if (strlen($this->value) >= 12) $score++;
        if (preg_match('/[A-Z]/', $this->value)) $score++;
        if (preg_match('/[a-z]/', $this->value)) $score++;
        if (preg_match('/[0-9]/', $this->value)) $score++;
        if (preg_match('/[^A-Za-z0-9]/', $this->value)) $score++;
