...
- Use `formatDateShort()` utility for all "Created" columns in tables.
- Format: `"Feb 16, 2026"` (Month first - English standard).
- Mandatory for all date columns unless a full timestamp is explicitly required.
...
- [ ] Every API endpoint route (`/api/{module}`) MUST be mapped in Swagger.
- [ ] Add the following Swagger Base annotation to the API Controller:
...
- [ ] By default, the main CRUD generation should target web inertia. Use the PowerShell generator drafts from `GENERATOR-CRUD/` to bootstrap.
...
| POST   | `/{module}/data/admin/bulk-delete`    | —                                    | Bulk delete        |
...
## 10. TanStack Table v8 — Correct Pattern

> **Critical rules — enforced on ALL table components:**
>
...
> 3. `columnHelper` MUST NOT appear in `useMemo` deps (it is a static constant)
> 4. Use `createColumnHelper<T>()`, `useReactTable()`, `flexRender()` — never wrapper abstractions that hide the TanStack API
...
export function {Entities}Table({
  data, isLoading, isError = false,
  onDelete, rowSelection, onRowSelectionChange,
}: {Entities}TableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([]);

  // ✅ columnHelper NOT in deps — it is a module-level constant
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            <Link href={`/{module}/${item.uuid}`}><Eye size={14} /></Link>
            {!isDeleted ? (
              <>
                <Link href={`/{module}/${item.uuid}/edit`}><Pencil size={14} /></Link>
                <button onClick={() => onDelete(item.uuid, item.name)}><Trash2 size={14} /></button>
              </>
            ) : (
              <button onClick={() => onRestore(item.uuid)}><CheckCircle size={14} /></button>
            )}
          </div>
        );
      },
...
### DataTable generic wrapper (shadcn-style)

When using the `<DataTable>` wrapper from `shadcn/data-table.tsx`, always pass `getRowId`:
...
<DataTable
    ...
    getRowId={(row) => row.uuid} // ✅ required — never rely on array index
/>
```
...
Every `{Entities}IndexPage.tsx` must include ALL of the following:

- **Counter Text**: `{count} records` (or singular/plural: `{count} {count === 1 ? 'record' : 'records'} found`).
- **Status Filter**: Dropdown with "All Status", "Active", "Deleted" options.
- **Bulk Delete**: `rowSelection` state + `DataTableBulkActions` + `router.post` (CSRF via Inertia).
- Search filter + Date range filter (`DataTableDateRangeFilter`) + Export (`ExportButton`).
- `DeleteConfirmModal` — never `window.confirm()`.
- `RestoreConfirmModal` — for soft-deleted rows.
- `useTransition` — wraps all search/filter/export state mutations (non-blocking).
- `useOptimistic` — instant delete feedback, MUST be called inside `React.startTransition(async () => { ... })`.
- `useRemember` (Inertia) — persists filters on back-navigation.
- Paginator with **sliding window** (5 pages around current page).
- Soft-deleted rows styled with `var(--deleted-row-bg)` token.
...
export default function {Entities}IndexPage(): React.JSX.Element {
  const [filters, setFilters] = useRemember<{Entity}Filters>({ page: 1, perPage: 15 }, '{entity}-filters');
  const [rowSelection, setRowSelection] = React.useState<RowSelectionState>({});
  const [pendingDelete, setPendingDelete] = React.useState<{ uuid: string; name: string } | null>(null);

  // React 19: non-blocking state updates
  ...
  // ✅ useOptimistic MUST be called inside startTransition async
  ...
  // Bulk delete — CSRF handled by Inertia router
  const selectedUuids = React.useMemo(
    () => Object.keys(rowSelection).filter(k => rowSelection[k as keyof typeof rowSelection]),
    [rowSelection]
  );
    ...
    <>
      ...
      <AppLayout>
        ...
        <StatusFilter
          ...
          onChange={status => startSearchTransition(() => setFilters(f => ({ ...f, status, page: 1 })))}
        />
        <DataTableDateRangeFilter
          dateFrom={filters.dateFrom}
          dateTo={filters.dateTo}
          onChange={range => startSearchTransition(() => setFilters(f => ({ ...f, ...range, page: 1 })))}
        />
        <ExportButton onExport={format => startExportTransition(() => {
        ...
        {selectedUuids.length > 0 && (
          <DataTableBulkActions count={selectedUuids.length} onDelete={handleBulkDelete} />
        )}

        {/* Table — pass optimisticItems, not items */}
        <{Entities}Table
          ...
          <div className="flex items-center gap-2">
            ...
            {pageWindow.map(p => (
              <button
                ...
                style={meta.currentPage === p ? { color: 'var(--accent-primary)' } : undefined}
              >{p}</button>
            ))}
            <button
            ...
            <AppLayout>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>
                        {data?.meta.total ?? 0} {data?.meta.total === 1 ? 'record' : 'records'} found
                    </p>
                </div>

                {/* Filters + Bulk Actions + Export toolbar */}
                <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                    <StatusFilter
                        value={filters.status ?? "active"}
                        onChange={status => startTransition(() => setFilters(f => ({ ...f, status, page: 1 })))}
                    />
...
### A01:2025 — Broken Access Control _(#1, includes SSRF)_
...
### A02:2025 — Security Misconfiguration _(moved up from #5)_
...
### A03:2025 — Software Supply Chain Failures _(new — expanded from Vulnerable Components)_
...
### A04:2025 — Cryptographic Failures _(down from #2)_
...
### A05:2025 — Injection _(down from #3 — includes SQLi, XSS, OS command injection)_
...
### A06:2025 — Insecure Design _(down from #4)_
...
### A07:2025 — Authentication Failures _(stable at #7)_
...
### A08:2025 — Software or Data Integrity Failures _(stable)_
...
### A09:2025 — Logging & Alerting Failures _(was "Security Logging and Monitoring Failures")_
...
### A10:2025 — Mishandling of Exceptional Conditions _(new)_
...
- [ ] Export route `/export` registered BEFORE `/{uuid}` to avoid routing collision

**Swagger / OpenAPI**

- [ ] `@OA\Info` + `@OA\Server` + `@OA\SecurityScheme(sanctum)` declared once
- [ ] Every API method has `@OA\Get`/`@OA\Post`/`@OA\Put`/`@OA\Delete`/`@OA\Patch`
- [ ] Every exposed DTO/Resource has `@OA\Schema`
- [ ] `l5-swagger:generate` run after any API controller change
...
- [ ] `queryKey` first element = entity name string (e.g. `['users', filters]`) — no `['companies']` for users ✅
- [ ] `DeleteConfirmModal` used for all deletes (no `window.confirm()`)
- [ ] `RestoreConfirmModal` used for all restores
- [ ] Counter Text follows pattern: `{count} {count === 1 ? 'record' : 'records'} found`
- [ ] Status Filter dropdown present (All/Active/Deleted)
- [ ] Bulk Delete implemented via `rowSelection` + `DataTableBulkActions` + `router.post`
- [ ] `formatDateShort()` used for all "Created" table columns
...
- [ ] `useTransition` wraps search/filter/export state updates ✅
- [ ] `useOptimistic` called inside `React.startTransition(async () => {...})` ✅
- [ ] `useRemember` (Inertia) used for filter persistence ✅

**TanStack Table v8 (enforced)**

- [ ] `columnHelper` defined **outside** the component (module-level constant) ✅
- [ ] `getRowId: (row) => row.uuid` provided to `useReactTable` ✅
- [ ] `columnHelper` NOT in `useMemo` deps ✅
- [ ] `DataTable` wrapper always receives `getRowId` prop ✅
- [ ] Pagination is a sliding window (5 pages around current), never a raw `Array.from({length: lastPage})` ✅
