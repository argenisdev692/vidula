...
type ParsedAddress = Pick<AddressValue, 'address' | 'zip_code' | 'city' | 'state' | 'country' | 'country_code'>;

/**
 * Convert the new Places API `addressComponents` array into our flat
 * {@link AddressValue} shape. `address`, `zip_code`, `city`, `state`, `country`
 * and `country_code` are derived here; `latitude`/`longitude` come from the place
...
export function parseAddressComponents(components: readonly google.maps.places.AddressComponent[]): ParsedAddress {
    ...
    const short = (type: string): string | null =>
        components.find((component) => component.types.includes(type))?.shortText ?? null;
    ...
    return {
        ...
        // ISO-3166 alpha-2 (e.g. "US"); Google returns it uppercased.
