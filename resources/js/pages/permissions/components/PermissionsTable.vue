...
/**
 * Permissions server-side DataTable. Fed by Inertia partial reloads (the parent
...
const emit = defineEmits<{
    ...
    'update:selection': [rows: Permission[]];
}>();
...
<template>
    <div class="crud-table-wrap">
        <DataTable
            ...
            @update:selection="(rows: Permission[]) => emit('update:selection', rows)"
        ...
        >
            ...
            <Column field="name" header="Permission">
                <template #body="{ data }">
                    <div class="perm-name">
                        ...
                        <span class="perm-name__text">{{ (data as Permission).name }}</span>
                    </div>
                </template>
            </Column>
            ...
            <Column field="guard_name" header="Guard">
                <template #body="{ data }">
                    <span class="mono">{{ (data as Permission).guard_name }}</span>
                </template>
            </Column>

            <Column field="created_at" header="Created">
                <template #body="{ data }">
                    <span class="mono">{{ formatDate((data as Permission).created_at) }}</span>
                </template>
            </Column>
                ...
                <template #body="{ data }">
                    <div class="actions-cell">
                        <PermissionGuard permission="VIEW_PERMISSIONS">
                            <Link
                                :href="`/permissions/${(data as Permission).uuid}`"
                        ...
                        <template v-if="(data as Permission).deleted_at">
                            <PermissionGuard permission="RESTORE_PERMISSIONS">
                                <button
                                    type="button"
                                    ...
                                    @click="emit('restore', data as Permission)"
                                >
                        ...
                        <template v-else>
                            <PermissionGuard permission="UPDATE_PERMISSIONS">
                                <button
                                    ...
                                    title="Edit"
                                    v-tooltip.top="'Edit'"
                                    @click="emit('edit', data as Permission)"
                                >
                            ...
                            <PermissionGuard permission="DELETE_PERMISSIONS">
                                <button
                                    ...
                                    @click="emit('delete', data as Permission)"
                                >
