...
/**
 * New campaign — the AI wizard itself (GET /campaigns/create, CREATE_CAMPAIGNS).
 * There is no manual "create" form: a campaign is always AI-born. Step 1 asks
 * the topic-ideation agent for up to 10 candidate Meta Ads angles (or the
 * user types one directly); Step 2 kicks off the async quality-loop job
 * (`GenerateCampaignJob`, up to 5 iterations) and returns immediately with a
 * `generating` row — this page then waits on the real-time progress channel
 * (with a manual status refresh fallback when Reverb isn't reachable) and
 * hands off to the Edit/review screen once the job finishes. Mirrors the
 * Social Media module's wizard precedent.
 */
...
const FUNNEL_STAGES: { label: string; value: FunnelStage }[] = [
    ...
    { label: 'Loyalty — retention', value: 'loyalty' },
];

const PLATFORMS: { label: string; value: CampaignPlatform }[] = [
    { label: 'Facebook', value: 'facebook' },
    { label: 'Instagram', value: 'instagram' },
    { label: 'Facebook + Instagram', value: 'both' },
];

const AD_FORMATS: { label: string; value: AdFormat }[] = [
    { label: 'Feed', value: 'feed' },
    { label: 'Story', value: 'story' },
    { label: 'Reel', value: 'reel' },
    { label: 'Carousel', value: 'carousel' },
    { label: 'Lead form (Instant Form)', value: 'lead_form' },
];
...
const PLATFORM_VALUES: CampaignPlatform[] = ['facebook', 'instagram', 'both'];
const AD_FORMAT_VALUES: AdFormat[] = ['feed', 'story', 'reel', 'carousel', 'lead_form'];

/** The agent returns a free-text `suggested_format` (e.g. "Carousel with 3 images") — loosely match it onto the enum. */
function matchAdFormat(suggested: string): AdFormat | null {
    const normalized = suggested.toLowerCase().replace(/\s+/g, '_');
    const exact = AD_FORMAT_VALUES.find((value) => normalized === value || normalized.includes(value));
    if (exact) {
        return exact;
    }
    return normalized.includes('lead') || normalized.includes('form') ? 'lead_form' : null;
}

function pickIdea(idea: CampaignTopicIdea): void {
    selectedIdea.value = idea;
    topicSteer.value = '';
    if (idea.funnel_stage) {
        funnelStage.value = idea.funnel_stage;
    }
    if (PLATFORM_VALUES.includes(idea.platform as CampaignPlatform)) {
        platform.value = idea.platform as CampaignPlatform;
    }
    const matchedFormat = matchAdFormat(idea.suggested_format);
    if (matchedFormat) {
        adFormat.value = matchedFormat;
    }
}
...
const platform = ref<CampaignPlatform>('both');
...
const activeProgress = computed(() =>
    generatedUuid.value && progress.value?.campaign_uuid === generatedUuid.value ? progress.value : null,
);

async function generateCampaign(): Promise<void> {
    if (!chosenTopic.value) {
        generateError.value = 'Pick a suggested angle or type a topic first.';
        return;
    }
...
function goToReview(): void {
    if (generatedUuid.value) {
        router.visit(`/campaigns/${generatedUuid.value}/edit`);
    }
}
...
<template>
    ...
    <AppHeader title="New campaign" subtitle="Pick an angle, then let the quality-loop agent build a scored Meta Ads package" />

    <PermissionGuard permission="CREATE_CAMPAIGNS">
        <template #fallback>
            <div class="empty">
                <i class="pi pi-lock" aria-hidden="true" />
                <p>You don't have permission to create campaigns.</p>
            </div>
        </template>
            ...
            <article class="card">
                ...
                <section v-if="generatedUuid" class="wizard-step">
                    ...
                    <p v-if="isDone" class="wizard-step__done">
                        <i class="pi pi-check-circle" aria-hidden="true" /> Campaign ready — review it now.
                    </p>
                ...
                <template v-else>
                    <section class="wizard-step">
                        <header class="wizard-step__head">
                            ...
                            <div>
                                <h2>Choose an angle</h2>
                                <p>Get Meta Ads angle ideas from your company profile and current trends, or type your own topic.</p>
                            </div>
                        </header>
                        ...
                        <ul v-if="ideas.length > 0" class="idea-list">
                            ...
                            >
                                ...
                                <div class="idea-card__meta">
                                    ...
                                    <span><i class="pi pi-users" aria-hidden="true" /> Leads {{ idea.estimated_lead_potential }}</span>
                        ...
                        <TextField
                            ...
                            placeholder="e.g. Free onboarding audit for ops teams struggling with activation"
                            :maxlength="255"
