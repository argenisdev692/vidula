...
/**
 * Weekly-rules server-side DataTable. Fed by Inertia partial reloads (the parent
 ...
 * project's CRUD DataTable reference).
 */
...
const props = defineProps<{
    ...
    first: number;
    loading: boolean;
    selection: AvailabilityRule[];
}>();

const emit = defineEmits<{
    edit: [rule: AvailabilityRule];
    delete: [rule: AvailabilityRule];
    restore: [rule: AvailabilityRule];
    page: [event: DataTablePageEvent];
    'update:selection': [rows: AvailabilityRule[]];
}>();
...
<template>
    <div class="crud-table-wrap">
        <DataTable
            ...
            @update:selection="(rows: AvailabilityRule[]) => emit('update:selection', rows)"
            @page="(event: DataTablePageEvent) => emit('page', event)"
        >
            <template #empty>
                <div class="table-empty">
                    <i class="pi pi-inbox" aria-hidden="true" />
                    <span>No availability rules match your filters.</span>
                </div>
            </template>

            <Column selection-mode="multiple" header-style="width: 3rem" :exportable="false" />

            <Column field="day_of_week" header="Weekday">
                <template #body="{ data }">
                    <span class="day-name">{{ dayLabel((data as AvailabilityRule).day_of_week) }}</span>
                </template>
            </Column>

            <Column field="start_time" header="Start">
                <template #body="{ data }">
                    <span class="mono">{{ formatTime((data as AvailabilityRule).start_time) }}</span>
                </template>
            </Column>

            <Column field="end_time" header="End">
                <template #body="{ data }">
                    <span class="mono">{{ formatTime((data as AvailabilityRule).end_time) }}</span>
                </template>
            </Column>
            ...
            <Column header="Availability">
                <template #body="{ data }">
                    <span
                        class="badge"
                        :class="(data as AvailabilityRule).is_available ? 'badge--available' : 'badge--blocked'"
                    >
                        {{ (data as AvailabilityRule).is_available ? 'Available' : 'Unavailable' }}
                    </span>
                </template>
            </Column>
            ...
            <Column field="created_at" header="Created">
                <template #body="{ data }">
                    <span class="mono">{{ formatDate((data as AvailabilityRule).created_at) }}</span>
                </template>
            </Column>
                ...
                <template #body="{ data }">
                    <div class="actions-cell">
                        <PermissionGuard permission="VIEW_AVAILABILITY_RULES">
                            <Link
                                :href="`/availability-rules/${(data as AvailabilityRule).uuid}`"
                        ...
                        <template v-if="(data as AvailabilityRule).deleted_at">
                            <PermissionGuard permission="RESTORE_AVAILABILITY_RULES">
                                <button
                                    type="button"
                                    ...
                                    @click="emit('restore', data as AvailabilityRule)"
                                >
                        ...
                        <template v-else>
                            <PermissionGuard permission="UPDATE_AVAILABILITY_RULES">
                                <button
                                    ...
                                    title="Edit"
                                    v-tooltip.top="'Edit'"
                                    @click="emit('edit', data as AvailabilityRule)"
                                >
                            ...
                            <PermissionGuard permission="DELETE_AVAILABILITY_RULES">
                                <button
                                    ...
                                    @click="emit('delete', data as AvailabilityRule)"
                                >
