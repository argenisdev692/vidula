...
export default function ProductIndexPage(): React.JSX.Element {
  const [filters, setFilters] = useRemember<ProductFilters>({ page: 1, perPage: 15 }, 'company-filters');
  ...
  async function handleExport(format: 'excel' | 'pdf'): Promise<void> {
    startExportTransition(() => {
      ...
      window.open(`/product/data/admin/export?${params.toString()}`, '_blank');
    });
  }
  ...
  function handleBulkDelete(): void {
    ...
    router.post('/product/data/admin/bulk-delete', { uuids: selectedUuids }, {
      onSuccess: () => {
        setRowSelection({});
