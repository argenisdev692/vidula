...
import { GraduationCap, Eye, Pencil, Trash2, CheckCircle } from 'lucide-react';
      ...
      cell: (info) => {
        ...
        return (
          <div className="flex items-center justify-end gap-2 pr-4">
            <Link href={`/students/${item.id}`} className="p-1.5 rounded-md hover:bg-(--bg-hover) text-(--text-secondary) transition-colors" title="View">
              <Eye size={16} />
            </Link>
            {!isDeleted ? (
              <>
                <Link href={`/students/${item.id}/edit`} className="p-1.5 rounded-md hover:bg-(--bg-hover) text-(--text-secondary) transition-colors" title="Edit">
                  <Pencil size={16} />
                </Link>
                <button
